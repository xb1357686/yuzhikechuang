...
const { confirm } = Modal;
const apiUrl = "https://luokangtao.vip/yzkc";
...
class Platform extends React.Component {
  ...
  handleOk = () => {
    ...
    form.validateFields(
      ["courseImg", "addCourseTitle", "courseDesc"],
      (error, values) => {
        if (!error) {
          const obj = {
            // courseId: 1,
        ...
        <Form layout="inline">
          ...
          >
            ...
            <Form.Item {...ItemLayout1} label="课时">
              ...
              <span>共{childrenList.length}节课时</span>
              {childrenList.map((res) => (
                <div key={res.key}>
                  <span style={{ marginRight: "30px" }}>
                    第{res.key + 1}节 {res.title}
                  </span>
                  <Icon
                    onClick={() => this.deleteList(res.key)}
                    style={{ color: "#018dcc", cursor: "pointer" }}
                    type="delete"
                  />
                </div>
              ))}
